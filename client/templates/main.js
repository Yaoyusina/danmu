...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$ctrlMain.append('<div class="ctrl-btn btn btn-primary play-btn" id="play-btn"><i class="fa fa-play" aria-hidden="true"></i></div>');
        ...
        //this.$ctrlMain.append('<div class="opt-btn ctrl-btn " ><i class="fa fa-sliders" aria-hidden="true"></i></div>');
        this.$ctrlMain.append('<input class="danmu-input ctrl-btn"   placeholder="按空格-暂停视频并输入" type="text" id="danmu_text" max=300 />'); // -> button
        this.$ctrlMain.append('<div class="send-btn  ctrl-btn"  >发送 ></div>');
        this.$ctrlMain.append('<div class="full-screen  ctrl-btn-right"><i class="fa fa-expand" aria-hidden="true"></i></div>');
        //this.$ctrlMain.append('<div class="loop-btn   ctrl-btn-right"><i class="fa fa-retweet" aria-hidden="true"></i></div>');
        //this.$ctrlMain.append('<div class="show-danmu  ctrl-btn-right ctrl-btn-right-active"><i class="fa fa-eye-slash" aria-hidden="true"></i></div>');
...
*/
        ...
        this.sendDanmu = function (e) {
            ...
            textObj = '{ "text":"' + text + '","color":"' + color + '","size":"' + size + '","position":"' + position + '","time":' + time  +',"videoId":"'+ videoId +'"}';
        ...
        this.playPause = function (e) {
            if (e.data.video.paused) {
                ...
                $(e.data.that.id + " #play-btn i").removeClass("fa-play").addClass("fa-pause");
                $(e.data.that.id + " #danmu_text").blur();
            }
            else {
                e.data.video.pause();
                $(e.data.that.id + " .danmu-div").danmu('danmuPause');
                $(e.data.that.id + " #play-btn i").removeClass("fa-pause").addClass("fa-play");
                $(e.data.that.id + " #danmu_text").focus();
            }
        };

       //主计时器
        var mainTimer = setInterval(function () {
            if($(that.id + " .danmu-video").length == 1){
               ...
               // }
           } //end if Uncaught IndexSizeError: Failed to execute 'end' on 'TimeRanges': The index provided (4294967295) is greater than the maximum bound (0).(anonymous function) @ main.js:214
        }, 1000);
        ...
        $(document).ready(function () {
            ...
            jQuery("body").keydown({video:$(".danmu-video")[0], that: that}, function (event) {

                if (event.which == 32 || event.which == 13){
                    that.playPause(event);
                    return false
                }
            });
        });
        ...
        //input 暂停、播放
        $(this.id + " .danmu-input").focus({video: this.video, that: that}, function (e){
            e.data.video.pause();
            $(e.data.that.id + " .danmu-div").danmu('danmuPause');
            $(e.data.that.id + " #play-btn i").removeClass("fa-pause").addClass("fa-play");
        });
       /* $(this.id + " .danmu-input").blur({video: this.video, that: that}, function (e){
            e.data.video.play();
            $(e.data.that.id + " .danmu-div").danmu('danmuResume');
            $(e.data.that.id + " #play-btn i").removeClass("fa-play").addClass("fa-pause");
        });*/
        ...
        $(this.id + " .full-screen").on("click", {video: this.video, that: that}, function (e) {
            if (!e.data.that.danmuPlayerFullScreen) {
                e.data.that.danmuDivposition = Math.round((($(window).height()-40)-$(window).width()*e.data.video.height/e.data.video.width)/2);
                //$css({"position":"fixed","zindex":"999","top":"0","left":"0","height":"100vh","width":"100vw"});
                $(e.data.that.id + " .full-screen i").removeClass("fa-expand").addClass("fa-compress");
                $(e.data.that.id).addClass("danmu-player-full-screen");
                e.data.that.danmuPlayerFullScreen = true;
                console.log($(e.data.that))
            }
            else {
                $(e.data.that.id + " .full-screen i").removeClass("fa-compress").addClass("fa-expand");
                $(e.data.that.id).removeClass("danmu-player-full-screen");
                e.data.that.danmuPlayerFullScreen = false;
                $(e.data.that.id + " .danmu-div").css("top","0");
                e.data.that.danmuDivposition = 0 ;
            }
//
        });
        ...
        //发送弹幕事件
        $(this.id + " .send-btn").on("click", {video: this.video, that: that}, function (e) {
